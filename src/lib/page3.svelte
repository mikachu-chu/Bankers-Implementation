<script lang="ts">
  ...
  export let max = Array.from({ length: +rows }, () => Array(+cols).fill(''));
   ...
   <form class="my-5 space-y-5 place-items-center" onsubmit={() => page = 4}>
      <h1 class="text-md font-bold text-center">Step 3: Fill out the Max Requirements Matrix</h1>
      <fieldset class="fieldset w-xs px-3 rounded-box bg-transparent border-2 border-[#8B79D9] shadow-md" >
      ...
      <div class="flex flex-col gap-1  overflow-x-auto pb-5">
        ...
        <div class="w-fit flex items-center ml-5">
          ...
          <input
            bind:value={max[i][j]}
            required
            class="rounded-xs mx-0.5 w-10 aspect-square text-center bg-white text-black"
            type="number"
            step="1"
            min="0"
            max="9"
    ...
    <div class="w-xs flex justify-between">
      <button class="btn btn-ghost border-0 bg-transparent" type="button" onclick={() => page = 2}>
